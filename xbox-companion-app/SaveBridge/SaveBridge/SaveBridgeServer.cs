...
namespace SaveBridge
{
    ...
    /// Endpoints:
    ///   GET  /status
    ///   GET  /wgs/list      — enumerate WGS filesystem (users / containers / blobs)
    ///   GET  /wgs/download?path=REL_PATH — raw WGS blob download
    ///   POST /wgs/upload?path=REL_PATH   — write raw bytes directly to WGS blob
    ///
    /// The WGS path format is: {XUID_hex}/{FOLDER_GUID}/{BLOB_GUID}
    /// Use /wgs/list first to discover the correct path for save_1.sav etc.
    /// </summary>
    public sealed class SaveBridgeServer
    {
        ...
        public async Task StartAsync()
        {
            ...
            // Bind on all interfaces so the Mac/PC can reach us over the network
                ...
                using (var outStream = args.Socket.OutputStream)
                {
                    if (path == "/status" && method == "GET")
                    {
                        await SendJson(outStream, 200,
                            "{\"status\":\"ok\",\"port\":" + PORT + ",\"build\":\"v9-wgsdirect\"}");
                    }
        ...
        //          <BLOB_GUID>        — raw blob data (gzip-compressed save)
        // ------------------------------------------------------------------ //
        ...
        private async Task HandleWgsList(IOutputStream outStream)
        {
            try
            {
                ...
                string wgsPath = Path.GetFullPath(
                    Path.Combine(localFolder.Path, "..", "SystemAppData", "wgs"));
        ...
        private async Task HandleWgsDownload(IOutputStream outStream, string relPath)
        {
            ...
            {
                var localFolder = Windows.Storage.ApplicationData.Current.LocalFolder;
                string wgsBase = Path.GetFullPath(
                    Path.Combine(localFolder.Path, "..", "SystemAppData", "wgs"));
